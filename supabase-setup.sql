...
-- ------------------------------------------------------------
-- On a real Supabase Cloud project, the platform automatically grants the
-- `anon`/`authenticated`/`service_role` roles standard table/sequence/routine
-- privileges as part of provisioning a new project -- that setup happens
-- outside of any SQL you run yourself, so it's easy to forget it needs to
-- exist at all. It's ALSO exactly what a bare local instance (e.g. a fresh
-- `supabase start` bootstrapped purely from this script, with no Supabase
-- Cloud project behind it) does NOT get for free -- RLS policies alone are
-- not sufficient; Postgres also requires the underlying GRANT before RLS
-- even gets a chance to evaluate a query. Without this block, every query
-- as `authenticated` (including from inside a SECURITY DEFINER helper like
-- is_admin()) fails outright with "permission denied for table X" instead
-- of a clean RLS rejection -- which is exactly the failure this project's
-- local pgTAP test suite hit (see supabase/tests/database/) before this
-- block was added.
--
-- Safe/idempotent to run against an existing Supabase Cloud project too --
-- it just re-states privileges that almost certainly already exist there.
